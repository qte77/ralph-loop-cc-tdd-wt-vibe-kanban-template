...
- [ ] **BUG: Agent can overwrite prd.json with test fixtures**: Story agents can write mock data to `ralph/docs/prd.json` (the orchestration state file) instead of isolated test paths. `commit_story_state()` then commits the corruption. **Fix**: validate story count hasn't decreased before committing, or make prd.json read-only during agent execution (restore after). Not BATS-specific — any story creating mock orchestration files can trigger this. Discovered during STORY-008 dogfooding (2026-03-25).
...
- [ ] **Strip inductive priors from template governance files**: `AGENTS.md` and `CONTRIBUTING.md` currently impose TDD discipline, core principles, and compound learning. These are opinions, not orchestration. Move all opinionated content out of committed governance files — users bring their own via CC plugins (`enabledPlugins` in `.claude/settings.local.json`). Template `AGENTS.md` keeps only: role boundaries, make commands, doc hierarchy. Template `CONTRIBUTING.md` keeps only: command reference, code patterns, PR guidelines. Self-feeding for dogfooding comes from our own plugins (`workspace-setup`, `python-dev`), not from template-imposed rules. Adapter TOML already handles tool abstraction; governance files should be equally agnostic.
